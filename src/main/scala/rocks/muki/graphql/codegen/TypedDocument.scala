...
object TypedDocument {
  ...
  case class Field(
      ...
      fragment: Option[FragmentSelection] = None,
    ...
    override def toString: String =
      s"Field(name:$name, type:${tpe.namedType.name}, selection: $selection , union: $union, fragment: $fragment)"
  ...
  case class Selection(
    ...
    def +(that: Selection) =
      Selection(
        (this.fields ++ that.fields).distinct,
        this.interfaces ++ that.interfaces
      )

    override def toString: String = s"Selection(fields: $fields, interfaces: $interfaces"
  }
  object Selection {
  ...
  /**
    * A `Fragment` is some meta data that is part of a `Selection`.
    *
    * @param name the of the fragment
    * @param typeCondition the `...on TypeCondition`
    */
  case class FragmentSelection(name: String, typeCondition: String) extends TypedDocument
  ...
  /**
    ...
    * @param field represents the actual field this fragment represents
    */
  case class Fragment(name: String, field: Field) extends Type
