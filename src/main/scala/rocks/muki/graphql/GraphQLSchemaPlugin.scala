...
object GraphQLSchemaPlugin extends AutoPlugin {
  ...
  override def projectSettings: Seq[Setting[_]] = Seq(
    ...
    graphqlSchemaGen / target := (Compile / target).value / "sbt-graphql",
    graphqlSchemaGen := {
      val schemaFile = (graphqlSchemaGen / target).value / "schema.graphql"
      runner.value.run(
        s"$packageName.$mainClass",
        Attributed.data((Compile / fullClasspath).value),
    ...
    (graphqlSchemaGen / name) := "build",
    graphqlSchemas += schema.GraphQLSchema(
      (graphqlSchemaGen / name).value,
    ...
    Compile / sourceGenerators += generateSchemaGeneratorClass()
  )
  ...
  private def generateSchemaGeneratorClass() = Def.task {
    ...
    val file = (Compile / sourceManaged).value / "sbt-sangria-codegen" / s"$mainClass.scala"
